...
Design and implement a high-performance Exercise Data Ingestion & API Platform that:

Fetches exercises from:

ExerciseDB Open Source (AGPL-3.0)

RapidAPI EDB (AscendAPI)

Normalizes & deduplicates data

Stores structured + media assets in a scalable database

Optimizes for production-level SaaS fitness platform

Supports 100k+ concurrent users

Is modular, maintainable, and cloud-ready

🏗 SYSTEM REQUIREMENTS
1️⃣ Data Ingestion Layer

Design a robust ETL pipeline that:

Fetches all exercises (11,000+ records)
...
Implement ingestion via:

Seed script (CLI based)

Background job (queue worker)

Cron scheduler for updates

Idempotent re-runnable sync

2️⃣ Database Architecture

Design a scalable relational schema (PostgreSQL preferred).
...
3️⃣ API Layer (Production Grade)

Build a REST or GraphQL API with:
...
1M+ stored exercises capacity

< 200ms P95 latency

5️⃣ Clean Architecture Requirements
...
Type-safe codebase (TypeScript preferred)

Proper logging (Winston / Pino)

Monitoring-ready (Prometheus hooks)
...
Lint + strict TS config

🧠 Advanced Engineering Expectations
...
🎖 BONUS (Elite Level Enhancements)

Vector embedding storage for AI search

Similar exercise recommendation engine

Automatic media optimization pipeline

Webhook for dataset updates

Admin panel for dataset moderation

📌 DATA SOURCES

ExerciseDB Open Source
https://github.com/ExerciseDB/exercisedb-api

RapidAPI AscendAPI
https://rapidapi.com/ascendapi/api/edb-with-videos-and-images-by-ascendapi

Ensure AGPL compliance if using open-source dataset.
...
* wellbeing (stress, mood, soreness, motivation)
