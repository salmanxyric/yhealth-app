  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Step number — pulse glow when active */}
        <motion.div
...
export function HowItWorksSection() {
  ...
  // Map scroll progress through the section to active step index
  const { scrollYProgress } = useScroll({
    target: scrollContainerRef,
    offset: ["start end", "end start"],
  });

  const pathProgress = useTransform(scrollYProgress, [0.15, 0.75], [0, 1]);

  useMotionValueEvent(scrollYProgress, "change", (value) => {
    // Map scroll progress to step index (0-3)
    if (value < 0.2) setActiveStep(-1);
    else if (value < 0.35) setActiveStep(0);
    else if (value < 0.5) setActiveStep(1);
    else if (value < 0.65) setActiveStep(2);
    ...
    >
      ...
      <div ref={sectionRef} className="container mx-auto px-4 relative z-10">
        ...
        <div className="relative">
          <ScrollDrawPathMotion progress={pathProgress} />
...
// ─── Motion-value driven path (re-renders on scroll) ────────────────
function ScrollDrawPathMotion({ progress }: { progress: MotionValue<number> }) {
