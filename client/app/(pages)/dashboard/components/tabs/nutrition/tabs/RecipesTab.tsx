    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4">
        <div className="flex items-center gap-2 sm:gap-3">
          <h3 className="text-[16px] sm:text-[18px] font-semibold text-white">My Recipes</h3>
          <span className="text-[13px] sm:text-sm text-slate-500">({recipes.length})</span>
        </div>
        <div className="flex items-center gap-2">
          {selectedRecipeIds.size > 0 && (
            <motion.button
              ...
              className="px-3 py-1.5 sm:py-2 rounded-xl bg-red-500/20 text-red-400 text-[13px] sm:text-sm font-medium flex items-center gap-2 hover:bg-red-500/30 transition-colors"
            >
          ...
          <motion.button
            ...
            className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl bg-emerald-500 text-white font-medium text-[13px] sm:text-sm flex items-center gap-2 hover:bg-emerald-600 transition-colors"
          >
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
        {categories.map((cat) => (
          <button
            ...
            className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-[13px] sm:text-sm font-medium whitespace-nowrap transition-colors ${
      ...
      {recipesLoading ? (
        ...
        <div className="rounded-2xl bg-slate-800/50 border border-slate-700/50 p-5 sm:p-8 text-center">
          <ChefHat className="w-10 h-10 sm:w-12 sm:h-12 text-slate-600 mx-auto mb-3 sm:mb-4" />
          <h4 className="text-white font-medium mb-2 text-[14px] sm:text-[15px]">No recipes yet</h4>
          <p className="text-slate-400 text-[13px] sm:text-sm mb-4">
            {showFavoritesOnly
          ...
          {!showFavoritesOnly && (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={onRecipeCreate}
              className="px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl bg-emerald-500 text-white font-medium text-[13px] sm:text-sm hover:bg-emerald-600 transition-colors"
            >
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredRecipes.map((recipe, index) => (
            <motion.div
              ...
              className={`rounded-2xl border p-3.5 sm:p-5 transition-all group cursor-pointer ${
            ...
            >
              ...
              <div className="w-full h-28 sm:h-32 rounded-xl bg-gradient-to-br from-emerald-500/20 to-green-500/10 mb-3 sm:mb-4 flex items-center justify-center relative overflow-hidden">
                {recipe.imageUrl ? (
              ...
              {/* Recipe Info */}
              <h4 className="font-semibold text-white mb-1 line-clamp-1 text-[14px] sm:text-[15px]">{recipe.name}</h4>
              {recipe.description && (
                <p className="text-[11px] sm:text-xs text-slate-400 mb-2 line-clamp-2">{recipe.description}</p>
              )}

              {/* Stats */}
              <div className="flex items-center gap-2 sm:gap-3 text-[11px] sm:text-xs text-slate-400 mb-3">
