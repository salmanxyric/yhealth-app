...
export default function ProfilePage() {
  ...
  const [, setGoalsStats] = useState<GoalsStats>({ total: 0, completed: 0, active: 0 });
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      // Fetch dashboard stats, health metrics, recent activities, goals, and achievements summary in parallel
      const [dashboardRes, healthRes, activityRes, goalsRes, achievementsRes] = await Promise.all([
        apiClient.get<{ data: DashboardStats }>('/stats/dashboard').catch((err) => {
          console.error('Error fetching dashboard stats:', err);
          return null;
        }),
        apiClient.get<{ data: HealthMetrics }>('/stats/health-metrics').catch((err) => {
          console.error('Error fetching health metrics:', err);
          return null;
        }),
        apiClient.get<{ data: { activities: RecentActivity[] } }>('/activity/recent?limit=4').catch((err) => {
          console.error('Error fetching recent activities:', err);
          return null;
        }),
        apiClient.get<{ data: { goals?: { status: string }[] } | { status: string }[] }>('/goals').catch((err) => {
          console.error('Error fetching goals:', err);
          return null;
        }),
        apiClient.get<{ data: { level: number; totalXP: number } }>('/achievements/summary').catch((err) => {
      ...
      if (healthRes?.data?.data) {
        const metrics = healthRes.data.data;
        setStats(prev => ({
          ...prev,
          caloriesBurned: metrics.calories?.burned || 0,
      ...
      if (activityRes?.data?.data?.activities) {
        setRecentActivities(activityRes.data.data.activities);
      } else {
      ...
      if (goalsRes?.data?.data) {
        ...
        const goalsArray = Array.isArray(goals) ? goals : (goals as { goals?: { status: string }[] }).goals || [];
        ...
        // Set default goals stats if API fails
        setGoalsStats({ total: 0, completed: 0, active: 0 });
      }

      // Process achievements summary
      if (achievementsRes?.data?.data) {
        ...
        setAchievementLevel({
          level,
          totalXP: achievementData.totalXP || 0,
          levelName,
        });
      }
    } catch (error) {
  ...
  function calculateHealthScore(metrics: HealthMetrics): number {
    ...
    if (metrics.calories?.burned > 0) score += 10;
    if (metrics.calories?.burned > 200) score += 10;

    // Add points for hydration
    if (metrics.water?.glasses >= 4) score += 10;
    if (metrics.water?.glasses >= 8) score += 5;

    // Add points for sleep
    if (metrics.sleep?.duration && metrics.sleep.duration >= 7) score += 10;
    if (metrics.sleep?.quality && metrics.sleep.quality >= 70) score += 5;
